...
namespace Bemagine.ServiceModel.Channels
{
    ...
    //--------------------------------------------------------------------------------------------//
    // ItemDequeuedCallback is called as an item is dequeued from the InputQueue.  The InputQueue 
    // lock is not held during the callback.  However, the user code is not notified of the item 
    // being available until the callback returns.  If you are not sure if the callback blocks for 
    // a long time, then first call IOThreadScheduler.ScheduleCallback to get to a "safe" thread.
    //--------------------------------------------------------------------------------------------//
    ...
    //--------------------------------------------------------------------------------------------//
    /// <summary>
    /// Handles asynchronous interactions between producers and consumers. Producers can dispatch 
    /// available data to the input queue, where it is dispatched to a waiting consumer or stored 
    /// until a consumer becomes available. Consumers can synchronously or asynchronously request 
    /// data from the queue, which is returned when data becomesavailable.
    /// </summary>
    /// <typeparam name="T">
    /// The concrete type of the consumer objects that are waiting for data.
    /// </typeparam>
    //--------------------------------------------------------------------------------------------//
    ...
    //--------------------------------------------------------------------------------------------//
        ...
        //----------------------------------------------------------------------------------------//
        /// <summary>
        /// Enumerates the possible states of the queue.
        /// </summary>
        //----------------------------------------------------------------------------------------//
    ...
    //--------------------------------------------------------------------------------------------//
        ...
        //----------------------------------------------------------------------------------------//
        // Each IQueueReader represents some consumer that is waiting for items to appear in the 
        // queue. The readerQueue stores them in an ordered list so consumers get serviced in a 
        // FIFO manner.
        //----------------------------------------------------------------------------------------//
        ...
        //----------------------------------------------------------------------------------------//
        // Each IQueueWaiter represents some waiter that is waiting for items to appear in the   
        // queue. When any item appears, all waiters are signaled.
        //----------------------------------------------------------------------------------------//

        private List<IQueueWaiter> waiterList;

        //----------------------------------------------------------------------------------------//
        // Represents the current state of the InputQueue as it transitions through its lifecycle.
        //----------------------------------------------------------------------------------------//
    ...
    //--------------------------------------------------------------------------------------------//
        ...
        public void EnqueueAndDispatch(Exception exception, ItemDequeuedCallback dequeuedCallback, 
        ...
        //----------------------------------------------------------------------------------------//
        // This does not block, however, Dispatch() must be called later if this function returns 
        // true.
        //----------------------------------------------------------------------------------------//
    ...
    //--------------------------------------------------------------------------------------------//
        ...
        //----------------------------------------------------------------------------------------//
        // WaitQueueReader class
        //----------------------------------------------------------------------------------------//

        private sealed class WaitQueueReader : IQueueReader
        {
            Exception exception;
        ...
        private sealed class AsyncQueueReader : AsyncResult, IQueueReader
        {
            ...
            public AsyncQueueReader(InputQueue<T> inputQueue, TimeSpan timeout, 
                    ...
                    : base(callback, state)
            {
                ...
                {
                    this.timer = 
                        new Timer(timerCallback, this, timeout, TimeSpan.FromMilliseconds(-1));
                }
            }
        ...
        //----------------------------------------------------------------------------------------//
        // WaitQueueWaiter class
        //----------------------------------------------------------------------------------------//

        private sealed class WaitQueueWaiter : IQueueWaiter
        {
            bool itemAvailable;
            ManualResetEvent waitEvent;
            object thisLock = new object();
        ...
        private sealed class AsyncQueueWaiter : AsyncResult, IQueueWaiter
        {
